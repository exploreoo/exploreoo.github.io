...
- ### diff算法优化
    ...
    - **Vue 2**：在比较新旧虚拟DOM树时，采用的是全量Diff（**深度优先遍历**+**前后双端对比**），逐层比较节点，这种方式在**处理大量节点时可能会有性能问题**

    - **Vue 3**：
      - **静态标记**：将静态节点到编译阶段进行标记，避免在每次渲染时重新创建这些节点或者标记部分可能变化的内容；
      - **缓存优化**：缓存一些VNode，避免重复创建；
      - **块级优化**：将模板分成多个块，每个块只包含动态内容，静态内容会被提升到块外部，这使得块内的比较更加高效；
      - **Diff的LIS（最长递增序列）**：通过key映射确定哪些节点可以稳定复用，然后通过**最长递增序列**找到最少DOM移动次数；
...
- Vue 3：支持多个 `v-model`（如 `v-model:title`、`v-model:content`），defineModel语法糖。
